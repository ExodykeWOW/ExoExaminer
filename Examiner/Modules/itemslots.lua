...
-- Options
ex.options[#ex.options + 1] = { var = "alwaysShowItemLevel", default = false, label = "Всегда показывать уровни предметов", tip = "Если эта функция включена, предметы всегда будут показывать свои уровни без необходимости удерживать клавишу ALT." };
...
local function OnEnter(self,motion)
	...
	elseif (self.realLink) then
		...
		gtt:AddLine("Этого предмета не было в локальном кэше предметов. Нажмите эту кнопку, чтобы перезагрузить кэш игрока, и предмет обновится.",1,1,1,1);
		gtt:Show();
	else
		gtt:SetText(_G[self.slotName:upper()]);
	end
...
local function OnClick(self,button)
	if (self.link) then
		if (button == "RightButton") then
			AzMsg("---|2 Обзор камней для "..select(2,GetItemInfo(self.link)).." |r---");
			for i = 1, 3 do
				local _, gemLink = GetItemGem(self.link,i);
				if (gemLink) then
					AzMsg(format("Камень |1%d|r = %s",i,gemLink));
				end
			end
