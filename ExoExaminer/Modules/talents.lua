...
mod:CreateButton("Таланты","Древо талантов","Посмотреть древо талантов игрока");
mod.canCache = true;
...
function mod.page:INSPECT_TALENT_READY(event)
	...
	mod.details:Add("Таланты");
	mod.details:Add("Специализация",combined > 0 and talentSpec or "Нет талантов");
	mod.details:Add("Расклад древа талантов",talentPoints);
	mod.details:Update();
...
function mod.MenuInit(parent,list)
	...
	list[1].text = "Специализация"; list[1].header = 1;
	list[2].text = "Активная"; list[2].value = nil; list[2].checked = (cfg.shownSpec == nil);
	list[3].text = (group == 1 and "|cff80ff80" or "").."Первая"; list[3].value = 1; list[3].checked = (cfg.shownSpec == 1);
	list[4].text = (group == 2 and "|cff80ff80" or noSecondary and "|cffff8080" or "").."Вторая"; list[4].value = 2; list[4].checked = (cfg.shownSpec == 2);
end
