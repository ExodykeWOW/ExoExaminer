...
-- Options
ex.options = {
	{ var = "makeMovable", default = false, label = "Сделать окно подвижным", tip = "Чтобы свободно перемещать аддона, включите эту опцию, иначе он будет вести себя как обычный фрейм, такой как Журнал Квестов или Книга Заклинаний." },
	{ var = "autoInspect", default = true, label = "Автопроверка при изменении цели", tip = "Если эта опция включена, аддон будет автоматически проверять вашу новую цель при ее изменении." },
	{ var = "clearInspectOnHide", default = false, label = "Очистить данные при скрытии", tip = "Когда окно аддона закрывается, эта опция очищает данные проверки, освобождая тем самым некоторую память." },
	{ var = "percentRatings", default = false, label = "Показать рейтинги в процентах", tip = "При включенной опции рейтинги будут отображаться в процентах относительно уровня проверяемого." },
	{ var = "combineAdditiveStats", default = true, label = "Объединение аддитивной статистики", tip = "Эта опция объединяет определенные характеристики, которые суммируются с другими. Сюда входит урон от заклинаний определенных школ заклинаний и АП для АП дальнего боя." },
};
...
-- Slash Help
ex.slashHelp = {
	" |2inspect <unit>|r = Проверяет данный юнит («цель», если юнит не указан)",
	" |2si <линк_предмета>|r = Сканирует один предмет и показывает общую сумму его объединенных характеристик.",
	" |2compare <линк_предмета1> <линк_предмета2>|r = Сравнивает два предмета, перечисляя различия в характеристиках.",
	" |2rating <статистика> <рейтинг> <уровень>|r = Конвертер рейтингов",
	" |2scale <значение>|r = Устанавливает масштаб окна аддона (по умолчанию — 1).",
	" |2clearcache|r = Очищает весь кеш аддона",
};
...
ex.slashFuncs = {
	...
	["si"] = function(cmd)
		if (cmd ~= "") then
			...
			AzMsg("--- |2Обзор сканирования для "..cmd.."|r ---");
	...
	["compare"] = function(cmd)
		if (cmd ~= "") then
			...
			if (item1 and item2) then
				...
				AzMsg("Не удалось проанализировать ссылки на элементы.");
			end
		else
			AzMsg("Ты не линканул предмет.");
		end
	...
	["rating"] = function(cmd)
		...
		if (stat and rating) then
			...
			AzMsg("Конвертированный рейтинг = |1"..(value or "Неверный Ввод"));
		else
			AzMsg("Неверный Ввод");
		end
	end,
...
SlashCmdList[modName] = function(cmd)
	...
	-- Invalid or No Command
		...
		AzMsg("Следующее |2параметры|r действительны для этого аддона:");
